...
solver.py
=========
The Single-Instance KINN Optimizer Engine (Iteration 2).
...
-----------------------------
The w_dual_pos hyperparameter is gone. The dual head is now Softplus-bounded
(model.py), so lambda_hat >= 0 structurally and loss.py no longer carries a
dual non-negativity term.
"""
...
def solve_kkt_instance(
    ...
    w_stat, w_prim, w_slack, w_primal_pos : float
        Weights for the 4 KKT physical loss components.
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # Calculate KKT Loss (dual non-negativity is structural via Softplus)
