...
preconditioning.py
==================
Adaptive Preconditioning Engine for KKT_Solver_Iteration_4_1.

Key Innovation:
- "auto" mode: Intelligently detects problem characteristics:
  * Sparse/asymmetric cost problems (e.g. sc50a, afiro, agg) -> Uses Row L2
    normalization to preserve delicate objective signals and natural coordinate travel.
  * Narrow corridor problems with high column imbalance and dense costs (e.g. blend)
    -> Uses Ruiz row-and-column equilibration to prevent corridor wall oscillations.
- Provides exact analytical unscaling transforms:
  x = C^{-1} * x_tilde
  lambda = s_c * R^{-1} * lambda_tilde
"""
...
def adaptive_preconditioning(
    ...
    # If cost is sparse/asymmetric (< 30%), or problem is massive wide-diameter, use row_l2
