...
Defaults:
- precond_method="auto": Intelligently picks Row L2 for sparse/wide problems (sc50a, afiro, agg)
  and Ruiz for narrow corridors (blend).
- anneal_mode="diameter_adaptive": Adjusts decay based on estimated polytope span.
- One-sided ReLU duality gap.
- Pure KKT residual checkpointing.
"""
...
from .loss import iteration_4_1_kkt_loss
...
def solve_kkt_instance(
    ...
    """
    Solves a KKT linear program instance using Iteration 4.3.1_Prac engine.
    """
    ...
    if verbose:
        print(f"\n[SOLVER] Solving '{kkt_system.name}' with Iteration 4.3.1_Prac...")
        print(f"   Variables: {n_vars} | Cons: {n_cons} | Precond: {precond_sys.method} | Anneal: {anneal_mode}")
        print(f"   Cost Scale: {precond_sys.cost_scale:.2e} | Est Span: {precond_sys.estimated_diameter:.2e}")
        print("   " + "-" * 90)
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        total_loss, metrics = iteration_4_1_kkt_loss(
            x_hat=x_hat,
